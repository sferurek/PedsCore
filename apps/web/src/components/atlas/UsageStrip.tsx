...
export function UsageStrip({ language, navigate }: { language: Language; navigate: (path: string) => void }) {
  ...
  return <section className="atlas-usage" aria-label={t.title}><div><span className="eyebrow">{atlas[language].usageEyebrow}</span><strong>{atlas[language].usage}</strong></div>{ready ? <><div><strong>{stats.totals.visitors.toLocaleString(language)}</strong><span>{t.totalVisitors}</span></div><div><strong>{stats.totals.last7DaysVisitors.toLocaleString(language)}</strong><span>{t.last7DaysVisitors}</span></div></> : <p role="status">{message}</p>}<button className="atlas-text-link" type="button" onClick={() => navigate(makePath(language, "stats", "global"))}>{translations[language].nav.stats} →</button></section>;
}
